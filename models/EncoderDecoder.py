...
class Encoder(nn.Module):
    def __init__(self, in_size=28, in_channels=1, c=64, latent_dims=10, latent_classes=1):
        ...
        self.conv1 = nn.Conv2d(
            in_channels=in_channels, out_channels=c, kernel_size=4, stride=2, padding=1
        )  # out: c x 14 x 14
        ...
        self.fc_mu = nn.ModuleList(
            [nn.Linear(c * 2 * img_dim * img_dim, self.latent_dims) for _ in range(latent_classes)]
        )
...
class Decoder(nn.Module):
    def __init__(self, out_size=28, out_channels=1, c=64, latent_dims=10, latent_classes=1):
        ...
        self.fc_in = nn.ModuleList(
            [nn.Linear(self.latent_dims, c * 2 * img_dim * img_dim) for _ in range(latent_classes)]
        )
...
class DecoderDCNN(nn.Module):
    def __init__(self, out_size=32, out_channels=3, latent_dims=100, fc_size=256, latent_classes=1):
        ...
        self.linear_layers = nn.Sequential(
            nn.Linear(self.fc_size, self.fc_size), nn.ReLU(), nn.Linear(self.fc_size, (c * 16) * 2 * 2), nn.ReLU()
        )
