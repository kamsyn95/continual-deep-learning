...
class MLP(BaseModel):
    ...
    def __init__(self, in_size=28, in_channels=1, fc_size=400, classes=10, tasks=2, scenario="task", multihead=True):
        ...
        self.hidden = nn.Sequential(
            nn.Linear(in_size * in_size * in_channels, fc_size), nn.ReLU(), nn.Linear(fc_size, fc_size), nn.ReLU()
        )
        if multihead:
...
class CNN(BaseModel):
    ...
    def __init__(
        self, in_size=28, in_channels=1, c=8, fc_size=128, classes=10, tasks=2, scenario="task", multihead=True
    ):
        super().__init__()
        ...
        if multihead:
            ...
            self.output = (
                nn.Linear(fc_size // 2, classes * tasks) if scenario == "class" else nn.Linear(fc_size // 2, classes)
            )
...
class DeepCNN(BaseModel):
    ...
    def __init__(
        self, in_size=32, in_channels=3, c=16, fc_size=256, classes=10, tasks=2, scenario="task", multihead=True
    ):
        super().__init__()
        ...
        if multihead:
            ...
            self.output = (
                nn.Linear(self.fc_size, classes * tasks) if scenario == "class" else nn.Linear(self.fc_size, classes)
            )
