...
def get_multitask_experiment(name, scenario, tasks, data_dir="./store/datasets", normalize=False, augment=False,
                             only_config=False, verbose=False, exception=False, only_test=False, mnist28=False):
    ...
    if name == 'permMNIST':
        ...
        if not only_config:
            # prepare dataset
            if not only_test:
                train_dataset = get_dataset(dataset_name, type="train", permutation=None, dir=data_dir,
                                            target_transform=None, verbose=verbose)
            test_dataset = get_dataset(dataset_name, type="test", permutation=None, dir=data_dir,
                                       target_transform=None, verbose=verbose)
        ...
        if not only_config:
            ...
            if not only_test:
                mnist_train = get_dataset(dataset_name, type="train", dir=data_dir, target_transform=target_transform,
                                          verbose=verbose)
            mnist_test = get_dataset(dataset_name, type="test", dir=data_dir, target_transform=target_transform,
                                     verbose=verbose)
