...
def get_dataset(
    ...
    # (e.g., not first all samples from class 1, then all samples from class 2, etc.)
...
def get_singletask_experiment(name, data_dir="./store/datasets", normalize=False, augment=False, verbose=False):
    ...
    trainset = get_dataset(
        data_type, type="train", dir=data_dir, verbose=verbose, normalize=normalize, augment=augment
    )
...
def get_multitask_experiment(
    ...
    if name == "permMNIST":
        # configurations
        dataset_name = "mnist28" if mnist28 else "mnist"
        ...
        if not only_config:
            ...
            if not only_test:
                train_dataset = get_dataset(
                    dataset_name, type="train", permutation=None, dir=data_dir, target_transform=None, verbose=verbose
                )
            test_dataset = get_dataset(
                dataset_name, type="test", permutation=None, dir=data_dir, target_transform=None, verbose=verbose
            )
        ...
        if not only_config:
            ...
            if not only_test:
                mnist_train = get_dataset(
                    dataset_name, type="train", dir=data_dir, target_transform=target_transform, verbose=verbose
                )
            mnist_test = get_dataset(
                dataset_name, type="test", dir=data_dir, target_transform=target_transform, verbose=verbose
            )
